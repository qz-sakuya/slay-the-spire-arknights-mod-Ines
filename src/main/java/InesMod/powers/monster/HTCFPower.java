...
import InesMod.monsters.CSZY;
import InesMod.monsters.DCSZY;
 ...
 * 死亡后召唤重生造物/大号重生造物
 */
public class HTCFPower extends AbstractInesPower {
    public static final String ID = PathHelper.nameToId(HTCFPower.class.getSimpleName());
    private static final PowerStrings powerStrings = CardCrawlGame.languagePack.getPowerStrings(ID); // 从游戏系统读取本地化资源

    private boolean spawnElite = false;

    public HTCFPower(AbstractCreature owner, int amount, boolean spawnElite) {
        super(ID,
                true,
                powerStrings,
                owner,
    ...
    public void onDeath() {
        if (this.owner.isDying){
            ...
            if(!spawnElite){
                ...
                newMonster = new DCSZY(this.owner.hb_x + this.owner.hb_y, MathUtils.random(-5.0F, 25.0F));
            }
