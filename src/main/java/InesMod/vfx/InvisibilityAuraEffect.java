...
public class InvisibilityAuraEffect extends AbstractGameEffect {
    private float x;
    private float y;
    private float vY;
    ...
    public InvisibilityAuraEffect(String stanceId) {
        this.img = ImageMaster.EXHAUST_L;
        this.duration = 2.0F;

        this.scale = MathUtils.random(2.7F, 2.5F) * Settings.scale;

        this.color = new Color(MathUtils.random(0.00F, 0.04F), MathUtils.random(0.0F, 0.01F), MathUtils.random(0.0F, 0.02F), 0.0F);
    ...
    public void update() {
        if (this.duration > 1.0F) {
            this.color.a = Interpolation.fade.apply(0.35F, 0.0F, this.duration - 1.0F);
        } else {
            this.color.a = Interpolation.fade.apply(0.0F, 0.35F, this.duration);
        }
